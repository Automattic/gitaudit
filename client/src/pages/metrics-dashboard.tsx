...
const COLORS = {
  positive: '#00a32a', // Green - improvement (decrease)
  negative: '#d63638', // Red - regression (increase)
  neutral: '#50575e', // Gray
  primary: 'var(--wp-admin-theme-color)',
  border: '#e0e0e0',
  cardBg: '#f6f7f7', // Inactive cards - gray
  cardActiveBg: '#fff', // Active card - white
};
...
function MetricCard({
  ...
  const { data, isLoading } = useQuery(
    perfAverageQueryOptions(owner, repo, metric.id)
  );
  ...
  if (data?.average && data?.previous) {
    ...
    if (Math.abs(change) >= 0.05) {
      if (higherBetter) {
        // For metrics like coverage, increase is good
        sentiment = change > 0 ? 'positive' : 'negative';
      } else {
        // For metrics like bundle size, build time - decrease is good
 ...
 */
const MetricChart = forwardRef<MetricChartHandle, {
  metric: Metric;
  owner: string;
  repo: string;
  limit: number;
  ...
  const externalTooltip = useCallback(
    (context: { chart: ChartJS; tooltip: { opacity: number; caretX: number; caretY: number; dataPoints?: Array<{ dataIndex: number }>; title?: string[]; body?: Array<{ lines: string[] }> } }) => {
  ...
  // Stable callback for zoom complete - use ref to avoid stale closure
  ...
  if (!perfData?.length) {
    return (
      <Notice status="info" isDismissible={false}>
        No performance data recorded yet for this metric. Use the API endpoint to
        log metrics from your CI/CD pipeline.
      </Notice>
    );
  }
...
function MetricsDashboard() {
  ...
  const { data: metrics, isLoading: metricsLoading } = useQuery(
    metricsQueryOptions(owner!, repo!)
  );
  ...
  // For GitHub repos, construct from owner/name; for custom repos, use the stored URL
  const repoUrl = repoStatus?.url || (repoStatus?.isGithub ? `https://github.com/${owner}/${repo}` : undefined);
    ...
    return (
      ...
      >
        <Notice status="warning" isDismissible={false}>
          No metrics configured yet. Go to{' '}
          <a href={`/repos/${owner}/${repo}/settings/metrics`}>Settings → Metrics</a>{' '}
          to configure the metrics you want to track.
        </Notice>
      </Page>
    );
  }
